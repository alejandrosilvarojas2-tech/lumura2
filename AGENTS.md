...
- **Separación de interfaces por rol** (`index.html`/`lumura.js`/`lumura.css`):
  ...
  - **Correos transaccionales en tiempo real** (`service/EmailService.java` + disparadores en `AliadoController`/`PedidoController`/`AdminController` + `service/MembresiaVencimientoJob.java`): se añadió `spring-boot-starter-mail`. `EmailService.enviar(destinatario, asunto, cuerpoHtml)` es `@Async`, envía HTML con plantilla de cabecera LUMURA, nunca lanza y, en **modo demo** (`app.mail.enabled=false`, default), solo registra `[MAIL][DEMO]` en el log (sin infraestructura SMTP). Para activar correos reales se definen por variables de entorno: `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD`, `MAIL_FROM`, `MAIL_ENABLED=true`. Disparadores: (1) **confirma membresía** (`POST /api/aliado/membresia`) → correo al aliado con el código y el vencimiento; (2) **compra confirmada** (`POST /api/pedidos`) → correo de confirmación al **cliente** (pedido #N, total, estado, referencia) y aviso "¡Vendiste!" a cada **aliado vendedor** cuyos productos se vendieron (mensaje entre roles, vía `DetalleCompra.idCatalogo → Catalogo.idAliado → Usuario.correo_usuario`); (3) **bloqueo/desbloqueo** de un usuario (`PUT /api/admin/usuarios/{id}/bloquear|desbloquear`) → correo al afectado con motivo/fecha (o reactivación). Con `@EnableScheduling`, `MembresiaVencimientoJob` corre cada 6 h: envía **recordatorio** ~5 días antes de vencer (deduplicado por id en memoria) y **aviso de bloqueo automático** a los aliados cuya `membresia_vence` ya pasó. No requiere cambios de esquema (reusa `correo_usuario`, `membresia_*`). Verificado E2E en servidor real (modo demo): los tres tipos de correo aparecen en el log 
...
- Suite de tests: 157 tests unitarios (JUnit 5 + Mockito) — `.\mvnw.cmd test`
