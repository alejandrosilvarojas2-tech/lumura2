...
- **`primeraApi/`** — proyecto Spring Boot con toda la API REST
  ...
  - `src/main/java/com/lumura/primeraApi/service/EmailService.java` — correo transaccional (SMTP, modo log/demo por defecto)
  - `src/main/java/com/lumura/primeraApi/service/MembresiaVencimientoJob.java` — job programado de recordatorio/bloqueo por vencimiento de membresía
  ...
  - `src/main/resources/application.properties` — conexión MySQL + JWT config + SMTP
...
| POST | `/api/auth/login` | No | Login, devuelve JWT + usuario. 403 si la cuenta está bloqueada (motivo + fecha fin); si el bloqueo venció, lo limpia y permite entrar. 403 si es ALIADO con membresía de distribuidor vencida ("Genera tu pago para continuar"). La respuesta incluye `membresia` (`codigo/plan/activada_en/vence`) cuando el usuario la tiene |
...
| GET | `/api/aliado/membresia` | JWT | Devuelve `{membresia: {codigo, plan, activada_en, vence}}` de la membresía de distribuidor del aliado (`{membresia: {}}` si no la tiene) |
| POST | `/api/aliado/membresia` | JWT | Confirma el pago (simulado) de la membresía con `{plan: basico|medio|premium}`. Genera y persiste el código `MEM-<idAliado>-<PLAN>-<aaaammdd>-<n4>` y el vencimiento (duración del plan + 1 día de gracia; basico=30d, medio=240d, premium=360d). 400 si el plan no es válido |
| DELETE | `/api/aliado/membresia` | JWT | Cancela la membresía de distribuidor: limpia `membresia_codigo/plan/activada_en/vence`, avisa al aliado por correo y permite reactivar pagando de nuevo. 400 si el aliado no tiene membresía activa |
...
| GET | `/api/admin/usuarios` | JWT Admin | Lista de usuarios (excluye al admin autenticado; incluye `bloqueado`, `motivo_bloqueo`, `bloqueo_hasta` y para el aliado `membresia_codigo`, `membresia_plan`, `membresia_activada_en`, `membresia_vence`; estos últimos `null` para no-ALIADO) |
...
- **Separación de interfaces por rol** (`index.html`/`lumura.js`/`lumura.css`):
  ...
  - **Ventana de membresías del aliado** (`index.html`/`lumura.js`): tras **publicar un artículo** (éxito de `publicarArticuloAliado`) el panel muestra `#aliado-panel-membresias` — pantalla **completa** dentro de `#screen-aliado` con 3 tarjetas (Básico $10.000/mes con 1er mes gratis, Medio $60.000 vigencia 8 meses con 2 meses gratis, Premium $90.000 vigencia 12 meses con publicidad aleatoria). Al pulsar un plan (`elegirPlanAliado(plan)` con el objeto `datosPlanes`) se abre el **`#modal-aviso-retracto`** (Aviso de Derecho de Retracto, texto del artículo 47 de la Ley 1480 de 2011); "Volver a los planes" (`cancelarRetracto`) regresa a las tarjetas y "Acepto y continuar al pago" (`aceptarRetracto`) muestra `#aliado-panel-pago` con el resumen dinámico `#pago-plan-detalle`. Enlace "Más tarde — Volver al Stock" para saltarla. El anuncio de membresías **se muestra una sola vez por aliado**: al confirmar el pago, `confirmarPagoAliado` persiste la llave `lumura_membresia_elegida_<id_usuario>`; `publicarArticuloAliado` consulta `membAliadoElegida()` para ir **directo a Stock** en publicaciones posteriores (si el aliado usa "Más tarde" sin escoger un plan, el anuncio vuelve a mostrarse en la siguiente publicación)
  - **Código de membresía + bloqueo automático** (backend `AliadoController/AuthController` + frontend `lumura.js`/`index.html`): `confirmarPagoAliado()` ahora es `async` y llama `POST /api/aliado/membresia` con `{plan: planAliadoPendiente}`; el servidor genera y persiste el **código `MEM-<idAliado>-<PLAN>-<aaaammdd>-<n4>`** (derivado del id del aliado) y el **vencimiento = duración del plan + 1 día de gracia** (basico 30d+1, medio 240d+1, premium 360d+1). El mensaje de éxito muestra el código y se guarda `membresia` en `localStorage` (`lumura_membresia_<id>`) y en `state.user`/`lumura_user` para el timer. La respuesta de login incluye `membresia` cuando existe. **Cierre en caliente**: `iniciarVigilanciaMembresiaAliado()` (al entrar al panel aliado, con guard anti-duplicado) corre un `setInterval` de 1s y al cruzar `vence` muestra "Tu membresía venció… cuenta bloqueada hasta generar el pago" y llama `cerrarSesion()`. **Backend**: el login de un ALIADO con `membresia_vence` pasado responde 403 "Cuenta bloqueada por membresía vencida… Genera tu pago para continuar." **Admin**: `GET /api/admin/usuarios` expone los campos de membresía; la fila de la tabla muestra badge **"Membresía vencida"** (naranja) para aliados vencidos; el botón "Ver" (`verPerfilUsuario`) añade en `#modal-perfil-usuario` el bloque `#perfil-memb-row` con **Código**, **Plan**, **Activada el**, **Vence el** y **cuenta regresiva** (`#perfil-memb-countdown`, formato `d HH:MM:SS`, `iniciarCountdownMembresia`), que al llegar a 0 muestra la nota "Membresía vencida — cuenta bloqueada automáticamente hasta que el aliado genere el pago"
  - **Correos transaccionales en tiempo real** (`service/EmailService.java` + disparadores en `AuthController`/`AliadoController`/`PedidoController`/`AdminController` + `service/MembresiaVencimientoJob.java`): se añadió `spring-boot-starter-mail`. `EmailService.enviar(destinatario, asunto, cuerpoHtml)` es `@Async`, envía HTML con plantilla de cabecera LUMURA, nunca lanza y, en **modo demo** (`app.mail.enabled=false`, default), solo registra `[MAIL][DEMO]` en el log (sin infraestructura SMTP). Para activar correos reales se definen por variables de entorno: `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD`, `MAIL_FROM`, `MAIL_ENABLED=true`. Cubre un disparador por cada evento relevante: **registro de cliente** (`POST /api/auth/register`) → bienvenida al nuevo usuario; **registro de aliado** (`POST /api/auth/register-aliado`) → bienvenida al negocio; **eliminar cuenta** (`DELETE /api/auth/cuenta`) → correo de despedida antes del borrado; **actualizar datos** (`PUT /api/auth/cuenta`) → confirmación de cambios; **compra confirmada** (`POST /api/pedidos`) → confirmación al **cliente** (pedido #N, total, estado, referencia) y aviso "¡Vendiste!" a cada **aliado vendedor** (mensaje entre roles, vía `DetalleCompra.idCatalogo → Catalogo.idAliado → Usuario.correo_usuario`); **cancelar compra** (`PUT /api/pedidos/{id}/cancelar`) → aviso de cancelación al cliente; **agregar producto** (`POST /api/aliado/productos`) → confirmación al aliado con precio/stock/código; **eliminar producto del catálogo** (`DELETE /api/admin/productos/{id}`) → aviso al **aliado dueño** del producto (nunca al admin); **pagar membresía** (`POST /api/aliado/membresia`) → correo al aliado con código y vencimiento; **cancelar membresía** (`DELETE /api/aliado/membresia`) → confirmación de cancelación; **bloqueo/desbloqueo** de un usuario (`PUT /api/admin/usuarios/{id}/bloquear|desbloquear`) → correo al afectado con motivo/fecha (o reactivación). **El admin nunca recibe correos de aliados ni de clientes** (ningún disparador envía al rol ADMIN). Con `@EnableScheduling`, `MembresiaVencimientoJob` corre cada 6 h: envía **recordatorio** ~5 días antes de vencer (deduplicado por id en memoria) y **aviso de bloqueo automático** a los aliados cuya `membresia_vence` ya pasó. No requiere cambios de esquema (reusa `correo_usuario`, `membresia_*`). Verificado E2E en servidor real (modo demo): los correos aparecen en el log 
...
- Suite de tests: 168 tests unitarios (JUnit 5 + Mockito) — `.\mvnw.cmd test`
- Git remote: `git@github.com:alejandrosilvarojas2-tech/lumura2.git`
- **Túnel público Cloudflare para demo (quick tunnel, sin tocar código/BD)**: con el servidor en `localhost:8080`, lanzar `cloudflared tunnel --url http://localhost:8080 --protocol http2 --no-autoupdate` (binario en `C:\Program Files (x86)\cloudflared\cloudflared.exe`; instalado por winget, sin cuenta). El flag `--protocol http2` es **obligatorio**: el firewall local bloquea el puerto 7844/QUIC de `region2`, por lo que el precheck interno marca `hard_fail`; con HTTP/2 por el puerto 443 el túnel conecta por `region1` y genera una URL `https://<aleatorio>.trycloudflare.com` (en stdout/stderr). La URL vive solo mientras `cloudflared` corra y el PC esté encendido (se cae y cambia al apagarse). No es necesario desactivar el firewall. Más detalle en `DOCUMENTACION_TECNICA.md` (entrada 25)
...
- Columnas de membresía en `usuario` (documentadas en `schema.sql`, creadas por JPA `ddl-auto=update`): `membresia_codigo` (VARCHAR 40), `membresia_plan` (VARCHAR 20), `membresia_activada_en` (DATETIME), `membresia_vence` (DATETIME). El vencimiento incluye el día de gracia; el login bloquea automáticamente a los aliados vencidos
