...
| DELETE | `/api/aliado/membresia` | JWT | Cancelar la membresía: limpia `membresia_codigo/plan/activada_en/vence` y avisa al aliado por correo (400 si no tiene membresía activa) |
...
| 24 | **Correos transaccionales en tiempo real (E2E, 31/08/2026)** | Se añadió `spring-boot-starter-mail` y el servicio `EmailService` (`enviar(destinatario, asunto, cuerpoHtml)`, `@Async`, plantilla HTML con cabecera LUMURA, nunca lanza). En **modo demo** (`app.mail.enabled=false` default) los correos NO se envían: se registran `[MAIL][DEMO]` en el log para no depender de infraestructura SMTP en dev; para activarlos se definen variables de entorno `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD`, `MAIL_FROM`, `MAIL_ENABLED=true` (proveedor tipo SendGrid/Mailgun/Brevo, `starttls` en 587). **Disparadores por evento**: (1) **registro cliente** `POST /api/auth/register` → bienvenida al nuevo usuario; (2) **registro aliado** `POST /api/auth/register-aliado` → bienvenida al negocio; (3) **eliminar cuenta** `DELETE /api/auth/cuenta` → correo de despedida antes del borrado; (4) **cambiar datos** `PUT /api/auth/cuenta` → confirmación de cambios; (5) **comprar** `POST /api/pedidos` → confirmación al **cliente** (pedido #N, total, estado, referencia) y aviso "¡Vendiste!" a cada **aliado vendedor** (mensaje entre roles, reusando `DetalleCompra.idCatalogo → Catalogo.idAliado → Usuario.correo_usuario`); (6) **cancelar compra** `PUT /api/pedidos/{id}/cancelar` → aviso de cancelación al cliente; (7) **agregar producto** `POST /api/aliado/productos` → confirmación al aliado (precio/stock/código); (8) **eliminar producto del catálogo** `DELETE /api/admin/productos/{id}` → aviso al **aliado dueño** del producto (el admin nunca recibe correo); (9) **pagar membresía** `POST /api/aliado/membresia` → correo al aliado con código y vencimiento; (10) **cancelar membresía** `DELETE /api/aliado/membresia` (nuevo endpoint que limpia `membresia_codigo/plan/activada_en/vence`) → confirmación de cancelación; (11) **bloqueo/desbloqueo** `PUT /api/admin/usuarios/{id}/bloquear|desbloquear` → correo al afectado (motivo+fecha / reactivación). **Regla clave**: el **admin nunca recibe correos de aliados ni de clientes** (ningún disparador apunta al rol ADMIN). **Aviso automático por "falta de pago"**: con `@EnableScheduling`, `MembresiaVencimientoJob` (cada 6 h) envía **recordatorio** ~5 días antes de que venza la membresía (deduplicado en memoria) y **aviso de bloqueo** a los aliados con `membresia_vence` pasada (reusa `membresia_*` de la entrada 23, sin cambios de esquema). 20 tests unitarios nuevos (EmailServiceTest 4, MembresiaVencimientoJobTest 2, AuthController +4, PedidoController +2, AliadoController +5, AdminController +3, incl. escenarios de correo) → **168 total**. Verificado en servidor real (modo demo): los correos de los distintos disparadores aparecen en el log `[MAIL][DEMO]` |

## 9.4 Evidencia de Pruebas Integrales (21/08/2026)

- Suite unitaria: **168/168 verde** (`.\mvnw.cmd test`)
...
- **Correos transaccionales en tiempo real (E2E, 31/08/2026)**: se añadió la capa de correo
  (`EmailService` + `spring-boot-starter-mail`) conectada a un disparador por evento relevante:
  registro de cliente/aliado, eliminar cuenta, cambiar datos, comprar, cancelar compra, agregar
  producto, eliminar producto del catálogo (aviso al aliado dueño), pagar/cancelar membresía y
  bloqueo/desbloqueo, más el job de vencimiento. El **admin nunca recibe correos de aliados ni
  de clientes**. Verificado en servidor real en **modo demo** (`MAIL_ENABLED=false`): los
  distintos disparadores registran `[MAIL][DEMO]` en el log con el destinatario correcto
  (aliado pagador de membresía, cliente comprador + aliado vendedor de cada compra, etc.).
  Suite unitaria 168/168 verde.
