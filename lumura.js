...
function renderAdminCatalogo() {
  ...
  tbody.innerHTML = state.productos.map((p, i) => {
    ...
    return '<tr><td>#' + String(p.id_catalogo || i + 1).padStart(3, '0') + '</td>' +
      ...
      '<td>' + (p.talla || '-') + '</td>' +
      '<td>' + badge + '</td>' +
      '<td><button class="btn-sm" style="background:#e3f2fd;color:#1565c0;margin-right:4px;">✏️</button>' +
      '<button class="btn-sm" style="background:#fce4ec;color:var(--accent);">🗑️</button></td></tr>';
  }).join('');
}
